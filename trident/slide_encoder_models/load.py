...
class ABMILSlideEncoder(BaseSlideEncoder):
    ...
    def _build(
        self, input_feature_dim, n_heads, head_dim, dropout, gated, pretrained=False
    ):
        ...
        pre_attention_layers = nn.Sequential(
            nn.Linear(input_feature_dim, 1536), nn.GELU(), nn.Dropout(0.1)
        )
...
class MeanSlideEncoder(BaseSlideEncoder):
    ...
    def _build(self, model_name="mean-default"):
        ...
        if model_name == "mean-conch_v1":
            ...
            print(
                f"\033[93mWARNING: Could not automatically infer embedding_dim for mean encoder {self.enc_name}. Setting to None.\033[0m"
            )
            embedding_dim = None
